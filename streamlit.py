...
# Title of the app
st.title("Generalized Data Analysis and Machine Learning App")
...
if uploaded_file is not None:
    ...
    try:
        ...
        features = st.sidebar.multiselect("Select Features for Analysis", data_info.columns)
        target = st.sidebar.selectbox("Select Target Variable (if applicable)", data_info.columns)
        ...
        if st.button("Show Histograms"):
            ...
            data_info[numeric_cols].hist(ax=ax, bins=20, figsize=(16, 12))
        ...
        if st.checkbox("K-Means Clustering"):
            if len(features) >= 2:
                ...
                fig, ax = plt.subplots()
                ax.scatter(X.iloc[:, 0], X.iloc[:, 1], c=data_info["cluster"], cmap="viridis")
